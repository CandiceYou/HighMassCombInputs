...
void normTempl(int spin=2 ,char*JEC ="nominal",  bool check=1){
...
cout<<process[i]<<endl;
        if (i==6) histSmNorm[i]=normalize(histSm[i],1,2000);
        else if (i==7) histSmNorm[i]=normalize(histSm[i],0,2000);
        else if (i==0 ||i==2 ||i==4 )histSmNorm[i]=normalize(histSm[i],1,1000);
        else histSmNorm[i]=normalize(histSm[i],1,1000);
