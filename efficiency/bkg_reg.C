...
//2e2mu VBF
                if(mode==1 && ch==2 && eleSel==0 && fabs(Z1Flav*Z2Flav)== zzflav && abs(ZZsel)>=100){
                                hreco->Fill(GenHMass,(genHEPMCweight * PUWeight * dataMCWeight*0.9965));
                }

                else if(((eleSel==0||ZZsel>=100)||(eleSel==2 && ZZsel==120)) && fabs(Z1Flav*Z2Flav)== zzflav){ //REG and RSE
...
//	 double M[n]={115,120,125,126,135,140,145,150,155,160,180,200,230,250,350,400,450,500,600,750,900,1000,1200,1400,1600,1800,2000,2200,2400,2600,2800,3000};
         ...
         const Int_t n=32;
         double M[n]={115,120,125,126,135,140,145,150,155,160,180,200,250,300,350,400,450,500,600,750,900,1000,1200,1400,1600,1800,2000,2200,2400,2600,2800,3000};
//        const Int_t n=15; 
...
void bkg_reg(){
	...
	//debug
        ...
        TGraphErrors* ggZZ_4mu = makegr(0,0,0,kRed,20,1);
	TGraphErrors* ggZZ_4e = makegr(0,0,1,kGreen,20,1);
	TGraphErrors* ggZZ_2e2mu = makegr(0,0,2,kBlue,20,1);

        TGraphErrors* VBF_4mu = makegr(1,0,0,kRed+2,23,1);
        TGraphErrors* VBF_4e = makegr(1,0,1,kGreen+2,23,1);
        TGraphErrors* VBF_2e2mu = makegr(1,0,2,kBlue+2,23,1);


TFile f ("bkg_reg_eff.root","recreate");
f.cd();
ggZZ_4mu->SetName("ggZZ_reg_4mu"); ggZZ_4mu->Write();
ggZZ_4e->SetName("ggZZ_reg_4e"); ggZZ_4e->Write();
ggZZ_2e2mu->SetName("ggZZ_reg_2e2mu"); ggZZ_2e2mu->Write();
VBF_4mu->SetName("VBF_reg_4mu"); VBF_4mu->Write();
VBF_4e->SetName("VBF_reg_4e"); VBF_4e->Write();
VBF_2e2mu->SetName("VBF_reg_2e2mu"); VBF_2e2mu->Write();
f.Close();
...
//           mg->GetXaxis()->SetRangeUser(200,3000);
           ...
           leg3->AddEntry(ggZZ_4mu,"ggZZ_bkg 4mu","pl");
           leg3->AddEntry(ggZZ_4e,"ggZZ_bkg 4e","pl");
           leg3->AddEntry(ggZZ_2e2mu,"ggZZ_bkg 2e2mu","pl");
           leg3->AddEntry(VBF_4mu,"VBF_bkg 4mu","pl");
           leg3->AddEntry(VBF_4e,"VBF_bkg 4e","pl");
           leg3->AddEntry(VBF_2e2mu,"VBF_bkg 2e2mu","pl");
           leg3->Draw();

	c2->Update();
	c2->SaveAs("/afs/cern.ch/user/c/cayou/www/HighMass/160726/bkgEfficiency_reg.png");
	c2->SaveAs("/afs/cern.ch/user/c/cayou/www/HighMass/160726/bkgEfficiency_reg.pdf");
}
