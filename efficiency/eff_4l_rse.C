...
//	int inputfiles_VBF[]={700};
	...
	if (mode!=2){ //ggH
		for (vector<TString>::const_iterator file = files_ggH.begin(); file!=files_ggH.end(); ++file){ 
                        candTree_gen->Add(inputDir+(*file));
	...
	TH1F *hreco = new TH1F("hreco","hreco",m,0,m);
        ...
        vector<float>* reweightingweights_gen=0;
        ...
        if(mode!=2) //ggH
                candTree_gen->SetBranchAddress("reweightingweights",&reweightingweights_gen);
...
//        for (int i = 0; i <100 ; i++) {
                ...
                float reweightingweight_gen=1;
                if(mode==0) reweightingweight_gen=reweightingweights_gen->at(0);
                else if(mode==1) reweightingweight_gen=reweightingweights_gen->at(7);
                hgen->Fill(GenHMass_gen,(genHEPMCweight_gen * PUWeight_gen * reweightingweight_gen));
         }
...
//        for (int i = 0; i <limit ; i++) {
     		...
     		if(genFinalState!=ch) continue;
                float reweightingweight=1;
                if(mode==0) reweightingweight=reweightingweights->at(0);
                else if(mode==1) reweightingweight=reweightingweights->at(7);
                if(((eleSel==0 && ZZsel>=100)||(eleSel==2 && ZZsel==120)) && fabs(Z1Flav*Z2Flav)== zzflav){ //REG and RSE
				hreco->Fill(GenHMass,(genHEPMCweight * PUWeight * dataMCWeight * reweightingweight));
		}
...
void eff_4l_rse(){
	...
	TGraphErrors* ggH_4e = makegr(0,2,1,kGreen,20,1);
	TGraphErrors* ggH_2e2mu = makegr(0,2,2,kBlue,20,1);
        TGraphErrors* ggH2bp_4e = makegr(1,2,1,kGreen,24,2);
        TGraphErrors* ggH2bp_2e2mu = makegr(1,2,2,kBlue,24,2);
        TGraphErrors* VBF_4e = makegr(2,2,1,kGreen+2,23,1);
        TGraphErrors* VBF_2e2mu = makegr(2,2,2,kBlue+2,23,1);

TFile f ("rse_eff.root","recreate");
f.cd();
ggH_4e->SetName("ggH_4e"); ggH_4e->Write();
ggH_2e2mu->SetName("ggH_2e2mu"); ggH_2e2mu->Write();
ggH2bp_4e->SetName("ggH2bp_4e"); ggH2bp_4e->Write();
ggH2bp_2e2mu->SetName("ggH2bp_2e2mu"); ggH2bp_2e2mu->Write();
VBF_4e->SetName("VBF_4e"); VBF_4e->Write();
VBF_2e2mu->SetName("VBF_2e2mu"); VBF_2e2mu->Write();
f.Close();
	...
	mg->Add(ggH_2e2mu);
        ...
        mg->Draw("APL");
           ...
           leg3->AddEntry(ggH_2e2mu,"ggH->0+ 2e2mu,RSE","pl");
           leg3->AddEntry(ggH_4e,"ggH->0+ 4e,RSE","pl");
           leg3->AddEntry(ggH2bp_2e2mu,"ggH->2b+ 2e2mu,RSE","pl");
           leg3->AddEntry(ggH2bp_4e,"ggH->2b+ 4e,RSE","pl");
           leg3->AddEntry(VBF_2e2mu,"VBF->0+ 2e2mu,RSE","pl");
           leg3->AddEntry(VBF_4e,"VBF->0+ 4e,RSE","pl");
           leg3->Draw();
/*
...
*/	   
	c2->Update();
	c2->SaveAs("/afs/cern.ch/user/c/cayou/www/HighMass/160726/sigEfficiency_rse.png");
	c2->SaveAs("/afs/cern.ch/user/c/cayou/www/HighMass/160726/sigEfficiency_rse.pdf");
}
